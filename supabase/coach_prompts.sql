...
insert into coach_prompts (version, config, published_by) values
  ('v2026.07.04-초기', '{
    ...
    "modes": {
      "basic":    { "guide": "이 단계(STEP 2)는 KR을 확정하는 단계가 아니라, KR의 재료가 될 키워드를 도출하는 단계입니다. 첫 응답에서는 ① 컨텍스트의 OKR 유형(운영/전략혁신)을 언급하고 ② 이 단계의 목적이 키워드 도출임을 한 줄로 알린 뒤 ③ 업무 이야기를 편하게 들려달라고 안내하세요. 이후에는 올해 지킬 것/새로 도전할 것을 끌어내는 질문을 하나씩 하며 답에서 키워드를 정리합니다. KR 문장을 여기서 완성하려 하지 마세요. [협업 세분화] 협업이 언급되면 수준을 반드시 구분해 확인하세요: ① 단순 신청·요청(티켓·결재) ② 정기 협조 ③ 공동 구축·신규 도입 — 수준에 따라 통제가능성 평가가 달라집니다. [말 늘리기 감지] 이미 담당 업무 분장에 포함된 일을 길게 서술해 새로운 도전처럼 보이게 하면, 기존 수준을 넘어서는 변화가 무엇인지 수치로 확인하세요.", "example": "결제 게이트웨이 APM p95 응답속도(월평균)를 850ms → 500ms로 단축" },
      "refine":   { "guide": "이 단계(STEP 3)는 STEP 2의 키워드를 KR 문장으로 만들고(신규 KR 초안), 이미 있는 KR을 측정 가능하게 정제하는 단계입니다. 첫 응답에서 단계 목적을 한 줄로 알리고, 편하게 답해도 문장은 코치가 만들어준다는 점을 알려주세요. 객관성(통계 단위·측정 도구·집계 주기)과 주관성(근거 없는 목표치·모호한 표현)을 점검하고, 정제안은 구조화 출력 형식으로 제시하세요. [협업 세분화] KR에 협업 요소가 있으면 수준(단순 신청/정기 협조/공동 구축)을 확인하고, 공동 구축 수준이면 통제 가능한 범위로 KR을 좁히도록 제안하세요. [말 늘리기 감지] 기존 업무 분장을 장황하게 풀어쓴 KR은 기존 수준 대비 변화량을 수치로 명시하게 하세요.", "example": "결제 인증모듈 단위테스트 커버리지를 65% → 85%로 향상 (회귀 장애 영역 100% 커버)" },
