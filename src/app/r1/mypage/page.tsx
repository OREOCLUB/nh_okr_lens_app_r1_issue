...
export default function R1MyPage() {
  ...
  if (!user || !wizard) {
    return (
      <RoleShell role="R1" title="마이페이지" subtitle="불러오는 중…">
        <div style={{ padding: 60, textAlign: "center", color: "#7C87A4", fontSize: 14 }}>프로필을 불러오는 중이에요…</div>
      </RoleShell>
    );
  }
  ...
  const setProfile = (patch: Partial<WizardState["profile"]>) =>
    setWizard((w) => (w ? { ...w, profile: { ...w.profile, ...patch } } : w));
  ...
  return (
    <RoleShell
      role="R1"
      title="마이페이지"
      subtitle={`${user.name} · ${user.dept} · ${user.team} · ${user.grade} ${p.jobSeries}`}
    ...
    >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 18, marginBottom: 22 }}>
        <div style={{ background: "linear-gradient(135deg, #F1F4FD, #fff 70%)", border: "1px solid #C5D0F7", borderRadius: 14, padding: "20px 22px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 11, marginBottom: 12 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14.5, fontWeight: 700, color: "#0F1A36" }}>{user.name} 님의 프로필 진행률</div>
              <div style={{ fontSize: 12, color: "#5B6685", marginTop: 2 }}>{total}개 항목 중 {filled}개 입력 완료{total - filled > 0 && ` · ${total - filled}개 보완 필요`}</div>
            </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        <FieldGroup icon="🏢" title="직무 · 직렬" desc="인사시스템에서 자동 동기화된 정보예요." status="done">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
            <div><label style={label}>본부 · 팀</label><input style={{ ...input, background: "#F9FAFC", color: "#5B6685" }} value={`${user.dept} · ${user.team}`} readOnly /></div>
            <div><label style={label}>직급</label><input className="mono" style={{ ...input, background: "#F9FAFC", color: "#5B6685" }} value={user.grade} readOnly /></div>
            <div>
              <label style={label}>직렬</label>
              <div style={{ display: "flex", gap: 4, padding: 4, background: "var(--page-bg)", borderRadius: 10 }}>
                {["SE", "PM", "SM"].map((s) => (
                  <button key={s} onClick={() => setProfile({ jobSeries: s })} className="mono" style={{ flex: 1, padding: "8px 10px", borderRadius: 7, background: p.jobSeries === s ? "#fff" : "transparent", color: p.jobSeries === s ? "#0F1A36" : "#7C87A4", fontWeight: p.jobSeries === s ? 700 : 500, fontSize: 12.5, border: "none", cursor: "pointer", boxShadow: p.jobSeries === s ? "0 1px 2px rgba(31,42,74,.08)" : "none" }}>{s}</button>
                ))}
              </div>
            </div>
          </div>
        ...
        <FieldGroup icon="📝" title="업무 분장 · 담당 업무" desc="OKR의 객관성 검증에 활용돼요. 실제 책임 업무 영역을 정확히 적어주세요." status={p.mainDuty.trim() ? "done" : "in-progress"}>
          <div style={{ display: "grid", gap: 14 }}>
            <div>
              <label style={label}>주요 업무 분장 <span style={{ color: "#D14343" }}>*</span></label>
              <textarea style={{ ...input, minHeight: 70, resize: "vertical", lineHeight: 1.55 }} value={p.mainDuty} onChange={(e) => setProfile({ mainDuty: e.target.value })} />
              <div style={hint}>AI 정제 단계에서 이 영역과 KR이 일치하는지 자동 확인해요.</div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <div><label style={label}>담당 시스템 / 도메인</label><input style={input} value={p.systems} onChange={(e) => setProfile({ systems: e.target.value })} placeholder="예: 결제 게이트웨이 · 인증모듈" /></div>
        ...
        <FieldGroup icon="📅" title="근속 정보" desc="입사년도·현 보직 발령일이 KR 도전성 평가 시 참고돼요." status="done">
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
            ...
            <div><label style={label}>현 보직 발령일</label><input className="mono" style={{ ...input, background: "#F9FAFC", color: "#5B6685" }} value="2022-03" readOnly /><div style={hint}>{user.team} 근속 <b style={{ color: "#0F1A36" }}>4년 4개월</b></div></div>
        ...
        <FieldGroup icon="🤝" title="사수 정보 · 인수인계" desc="1년 이내 팀 이동 시 이전 담당자를 지정하면 그분의 지난 OKR을 참고할 수 있어요. (평가 점수는 비공개 · 열람은 P2)" status="done">
          <div style={{ border: "1px solid #BBE9CC", borderRadius: 12, padding: "16px 18px", background: "linear-gradient(135deg, #F4FBF7, #fff 70%)", display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: "#5B6685", marginTop: 2 }}>{user.team} (現 SRE팀 전배) · 인계일 2024.06.30</div>
            </div>
        ...
        <FieldGroup icon="🏠" title="근무 형태" desc="상주/파견 여부에 따라 협업 범위와 평가 기준이 달라질 수 있어요." status="done">
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            {([["onsite", "상주 (본사)", "본사 사무실 출근"], ["dispatch", "파견", "외부 사이트 상주"], ["remote", "원격", "재택 또는 거점"]] as const).map(([v, l, s]) => {
              const on = p.workType === v;
              return (
                <div key={v} onClick={() => setProfile({ workType: v })} style={{ flex: 1, minWidth: 150, padding: "13px 15px", background: on ? "#F1F4FD" : "#fff", border: `1.5px solid ${on ? "#3B5BDB" : "#E1E5EF"}`, borderRadius: 11, cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div style={{ marginTop: 28, padding: "18px 22px", background: "#fff", border: "1px solid #E6E9E4", borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: "var(--shadow-xs)" }}>
        <div style={{ fontSize: 13, color: "#5B6685" }}>마지막 저장 · <span className="mono">{savedAt ? new Date(savedAt).toLocaleString("ko-KR") : "아직 저장 전"}</span></div>
        <Button variant="primary" style={{ background: "#0A1F17", padding: "10px 22px" }} onClick={save}>변경사항 저장</Button>
      </div>
    </RoleShell>
  );
}
