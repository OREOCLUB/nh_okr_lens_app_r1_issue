...
const REVIEW_DATA: Record<string, Record<string, Review>> = {
  kr1: {
...
// 목데이터가 없는 KR용 결정적 합성 리뷰 (KR 신호 기반)
function synthReview(kr: WizardKR, vendorIdx: number): Review {
  const hasTool = !!kr.measureTool.trim();
  const hasNum = /\d/.test(kr.baseline) && /\d/.test(kr.goal);
  const base = 6 + (hasTool ? 1.5 : 0) + (hasNum ? 1.5 : 0) - vendorIdx * 0.5;
  const verdict = base >= 8 ? "pass" : base >= 6.5 ? "warn" : "fail";
  return {
    score: Math.round(base * 10) / 10,
    scoreLabel: base >= 8.5 ? "Strong" : base >= 7.5 ? "Good" : "Acceptable",
    verdict,
    summary: hasNum
      ? "측정 수치가 명확해요. 측정 도구와 집계 주기를 함께 명시하면 더 견고해져요."
      : "목표 수치를 baseline → goal 형태로 명시하면 객관성이 크게 올라가요.",
    items: [
      { c: "측정성", v: hasNum ? "pass" : "warn", note: hasNum ? "수치 목표 명확" : "수치 명시 권장" },
      { c: "증빙", v: hasTool ? "pass" : "warn", note: hasTool ? kr.measureTool : "측정 도구 지정 필요" },
      { c: "도전성", v: "pass", note: "개선 목표 확인" },
    ],
    suggestion: hasTool ? "이대로 진행해도 좋아요. 등급 구간이 겹치지 않는지만 확인해주세요." : "측정 도구를 지정하고 등급 기준을 검토해주세요.",
  };
}
...
export function Step6({ state, set, focusKrId }: { state: WizardState; set: (fn: (s: WizardState) => WizardState) => void; focusKrId?: string | null }) {
  ...
  const warnCt = allReviews.filter((r) => r.verdict === "warn").length;
  const failCt = allReviews.filter((r) => r.verdict === "fail").length;
  const avgScore = allReviews.length ? (allReviews.reduce((a, r) => a + r.score, 0) / allReviews.length).toFixed(1) : "—";
  const chosenCount = krs.filter((k) => k.chosenAI).length;

  const choose = (krId: string, vendorId: string, suggestion: string) =>
    set((s) => ({
      ...s,
      krs: s.krs.map((k) =>
        k.id === krId
          ? k.chosenAI === vendorId
            ? { ...k, chosenAI: null, aiSuggestion: "" }
            : { ...k, chosenAI: vendorId, aiSuggestion: suggestion }
          : k
      ),
    }));

  if (!cur) {
    return <div style={{ padding: 40, background: "#fff", border: "1px solid #E1E5EF", borderRadius: 16, textAlign: "center", color: "#7C87A4", fontSize: 13.5 }}>검토할 KR이 아직 없어요. 이전 단계를 먼저 완성해주세요.</div>;
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.3fr 1fr 1fr", gap: 14 }}>
        <div style={{ background: "linear-gradient(135deg, #0A1F17, #14342B)", color: "#fff", borderRadius: 16, padding: "20px 22px" }}>
          ...
          <div style={{ fontSize: 11, color: "#9DB3A9", marginTop: 2 }}>KR {krs.length}개 × AI 3사 = {krs.length * 3}건 검토 (목데이터 · 실 API는 P2)</div>
        ...
        <div style={{ background: "#fff", border: "1px solid #E1E5EF", borderRadius: 16, padding: "20px 22px" }}>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 6, margin: "12px 0" }}>
            <span className="mono" style={{ fontSize: 30, fontWeight: 700, color: "#3B5BDB" }}>{chosenCount}</span>
            <span style={{ fontSize: 14, color: "#7C87A4" }}>/ {krs.length}</span>
          </div>
        ...
        <div style={{ background: "#fff", border: "1px solid #E1E5EF", borderRadius: 16, padding: "20px 22px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
            {AI_VENDORS.map((v, i) => {
              const scores = krs.map((k) => reviewFor(k, v.id, i).score);
          ...
          return (
            <button key={k.id} onClick={() => setActiveId(k.id)} style={{ width: 250, flexShrink: 0, textAlign: "left", padding: "12px 14px", background: on ? "#F1F4FD" : "#fff", border: `1.5px solid ${on ? "#3B5BDB" : "#E1E5EF"}`, borderRadius: 11, cursor: "pointer", fontFamily: "var(--font-sans)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                {k.chosenAI && <span style={{ marginLeft: "auto", color: "#2F9E5E", fontSize: 10.5, fontWeight: 700 }}>채택 ✓</span>}
              </div>
              <div style={{ fontSize: 12, color: "#3A4565", marginTop: 6, lineHeight: 1.4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{k.kr}</div>
            </button>
          );
        })}
      </DragScroll>
