...
export function Step4({ state, set }: { state: WizardState; set: (fn: (s: WizardState) => WizardState) => void }) {
  const krs = state.krs;
  const [activeKR, setActiveKR] = useState(krs[0]?.id ?? "");
  const active = krs.find((k) => k.id === activeKR) ?? krs[0];
  const rec = active ? recommendFormat(active) : null;

  const setFormat = (id: string, format: string) =>
    set((s) => ({ ...s, krs: s.krs.map((k) => (k.id === id ? { ...k, format } : k)) }));

  const applyAll = () =>
    set((s) => ({ ...s, krs: s.krs.map((k) => ({ ...k, format: recommendFormat(k).format })) }));
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            return (
              <button key={k.id} onClick={() => setActiveKR(k.id)} style={{ flex: 1, textAlign: "left", padding: "12px 14px", background: on ? "#F1F4FD" : "#fff", border: `1.5px solid ${on ? "#3B5BDB" : "#E1E5EF"}`, borderRadius: 11, cursor: "pointer", fontFamily: "var(--font-sans)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                  ...
                  {k.format && <span style={{ padding: "1px 8px", borderRadius: 999, background: "#E5EBFB", color: "#213A8C", fontSize: 10, fontWeight: 700 }}>{k.format}</span>}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        {FORMATS.map((f) => {
          const sel = active.format === f.key;
          const isRec = rec?.format === f.key;
          return (
            <div key={f.key} onClick={() => setFormat(active.id, f.key)} style={{ position: "relative", background: sel ? "#F9FBFF" : "#fff", border: `2px solid ${sel ? f.accent : "#E1E5EF"}`, borderRadius: 16, padding: "20px 22px", cursor: "pointer", boxShadow: sel ? `0 0 0 4px ${f.accent}18` : "var(--shadow-xs)" }}>
              {isRec && <div style={{ position: "absolute", top: -10, left: 18, padding: "3px 10px", background: f.accent, color: "#fff", borderRadius: 999, fontSize: 10.5, fontWeight: 700 }}>이 KR엔 추천</div>}
      ...
      {/* AI recommendation panel — KR 신호 기반 결정적 추천 */}
